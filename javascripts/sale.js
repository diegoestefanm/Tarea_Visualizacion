/* function calc() {
    ...
    window.alert(`${price}círculoPero${number}Piezas. Subtotal.${price * number}círculoes`);
  } 
...
function calc() {
    const price = document.getElementById("product").value;
    const number = document.getElementById("number").value;
    window.alert(`El precio es${price}por la cantidad de ${number}. El subtotal es${price * number}yenes`);
  } 
 function obtenerProductorPorID(id) {
